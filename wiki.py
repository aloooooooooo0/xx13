...
#             if not('==' in r):
#                 if(len((r.strip()))>3):
#                    wikitext2 = wikitext2+r+'.'
#             else:
#                 break
#         wikitext2 = re.sub('\([^()]*\)', '', wikitext2)
#         wikitext2 = re.sub('\([^()]*\)', '', wikitext2)
#         wikitext2 = re.sub('\{[^\{\}]*\}', '', wikitext2)
#         bot.send_message(chat_id = message.chat.id, text = wikitext2)
#     except Exception as e:
#         bot.send_message(chat_id = message.chat.id, text ='В энциклопедии нет информации об этом')
#
#
